...
def ingresar_usuario():
    ...
    with conn.cursor() as cursor:
        cursor.execute("SELECT Usuario FROM Lectura WHERE Usuario = %s", (usuario,))
        if not cursor.fetchone():
            cursor.execute("INSERT INTO Lectura (Usuario, Libro, Autor, Fecha, Paginas_leidas, Meta_paginas) VALUES (%s, %s, %s, %s, %s, %s)", (usuario, "", "", None, 0, 0))
            conn.commit()
...
def menu():
    print("1. Agregar un nuevo libro leído")
    print("2. Ver el historial de libros leídos")
...
def agregar_libro(usuario):
    libro = input("Por favor, introduzca el nombre del libro: ")
